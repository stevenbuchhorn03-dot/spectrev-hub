const config = {
  ...
  function createGameView(canvas, glassElements, resolutionScale = config.resolutionScale) {
    ...
    const maxBlurPercent = 0.92;
    function render() {
      const canvasRect = canvas.getBoundingClientRect();
      gl.clear(gl.COLOR_BUFFER_BIT);
      gl.useProgram(programInfo.program);
      const maxW = Math.floor(gl.canvas.width * maxBlurPercent);
      const maxH = Math.floor(gl.canvas.height * maxBlurPercent);
      ...
      glassElements.forEach(element => {
        ...
        const buffer = 1;
        let scaledLeft = Math.floor((rect.left - canvasRect.left) * currentResolutionScale) - buffer;
        let scaledTop = Math.floor((rect.top - canvasRect.top) * currentResolutionScale) - buffer;
        let scaledWidth = Math.ceil(rect.width * currentResolutionScale) + buffer * 2;
        let scaledHeight = Math.ceil(rect.height * currentResolutionScale) + buffer * 2;
        if (scaledWidth > maxW || scaledHeight > maxH) {
        ...
        const isCircle = String(style.borderRadius).includes('%') && parseFloat(style.borderRadius) >= 50;

        const halfW = scaledWidth / 2;
        const halfH = scaledHeight / 2;
        gl.uniform4f(programInfo.uniformLocations.shape, centerX, centerY, halfW, isCircle ? 0 : halfH);
  ...
  document.addEventListener('DOMContentLoaded', () => {
    const allWithHlsl = Array.from(document.querySelectorAll('[data-hlsl]'));
    const glassElements = allWithHlsl.filter((el) => !el.getAttribute('data-hlsl-layer'));
    const overlayGlassElements = allWithHlsl.filter((el) => el.getAttribute('data-hlsl-layer') === 'overlay');
    ...
    const filterValid = (list) => list.filter((el) => {
      if (!el.isConnected) return false;
      const rect = el.getBoundingClientRect();
      const maxSide = Math.max(window.innerWidth, window.innerHeight) * 0.92;
      if (rect.width > maxSide || rect.height > maxSide) return false;
      return true;
    });

    let gameView = null;
    let overlayGameView = null;
    ...
    if (glassElements.length > 0) {
      ...
      glassElements.forEach(prepareElement);
      gameView = createGameView(canvas, glassElements);
      if (gameView) {
        gameView.resize(window.innerWidth, window.innerHeight);
        gameView.start();
      }
    ...
    if (overlayGlassElements.length > 0) {
      const overlayCanvas = document.createElement('canvas');
      overlayCanvas.id = 'render-overlay';
      ...
      if (inventoryRoot && itemTooltip) {
        inventoryRoot.insertBefore(overlayCanvas, itemTooltip);
      } else {
        document.body.appendChild(overlayCanvas);
      }
      overlayGlassElements.forEach(prepareElement);
      overlayGameView = createGameView(overlayCanvas, overlayGlassElements);
      if (overlayGameView) {
        overlayGameView.resize(window.innerWidth, window.innerHeight);
        overlayGameView.start();
      }
    }

    if (gameView || overlayGameView) {
      document.body.classList.add('hlsl-active');
      const updateSize = () => {
        if (gameView) gameView.resize(window.innerWidth, window.innerHeight);
        if (overlayGameView) overlayGameView.resize(window.innerWidth, window.innerHeight);
      };
      window.addEventListener('resize', updateSize);

      window.hlslRefresh = () => {
        const all = Array.from(document.querySelectorAll('[data-hlsl]'));
        const current = filterValid(all);
        current.forEach(prepareElement);
        const main = current.filter((el) => el.getAttribute('data-hlsl-layer') !== 'overlay');
        const overlay = current.filter((el) => el.getAttribute('data-hlsl-layer') === 'overlay');
        glassElements.length = 0;
        glassElements.push(...main);
        overlayGlassElements.length = 0;
        overlayGlassElements.push(...overlay);
      };
    } else {
